...
from nlp_ptbr.experiment import ContextualEmbeddingsExperiment, Experiment, EmbeddingsExperiment
...
def parse_args():
    ...
    parser.add_argument('--model_name', type=str, dest='model_name', default='mbert', help='Transformers model name.')
...
def main():
    ...
    if do == 'both' or do == 'evaluate':
        ...
        wandb.init(
            entity="bertweetbr",
            notes=f'Contextual word-embeddings experiment for short text classification: {classes}',
            ...
            for k, v in ContextualEmbeddingsExperiment.checkpoints.items():
                if k in selected_transformers:
                    ...
                    experiment = ContextualEmbeddingsExperiment(
                        ...
                        experiment_name=f"{experiment_name}-{v['model_name']}",
                    ...
                    results, elapsed_time, dataset_eval_times = experiment.run_cv_datasets_evaluation(
                        datasets=datasets, index=datasets_index, text_col=col, disable_tqdm=disable_tqdm, only_these_folds=only_these_folds, batch_size=batch_size, save=True, eval_type=eval_type)
                    
                    all_results.append(ContextualEmbeddingsExperiment.results_to_pandas(
                        results, origin='pre-trained', experiment_type=experiment_type, text_col=col, model_name=ContextualEmbeddingsExperiment.checkpoints[k]['model_name']))
